...
function SignInForm({ buttonClassName, fullWidth, onSwitchToSignUp, onClose }: SignInFormProps) {
  ...
  async function handleSignIn() {
    ...
    try {
      ...
      dispatchToast({ title: 'Sign in failed', description: msg, variant: 'error' });
        return;
      }
      ...
      // Force the dashboard full-screen loader once right after login
      try {
        if (typeof window !== 'undefined') {
          localStorage.setItem('forceDashboardLoaderOnce', '1');
        }
      } catch {}
  ...
  // Removed transition completion handler
    ...
    <>
      <ToastProvider>
        ...
        <div className="relative">
          <input 
            ...
            inputMode="email"
          />
          <label htmlFor="signin-email" className="pointer-events-none absolute left-3 top-3 text-sm text-neutral-800 transition-all bg-white px-1.5 rounded-md shadow-sm peer-focus:-top-2 peer-focus:text-xs peer-focus:text-green-600 peer-placeholder-shown:top-3 peer-placeholder-shown:text-sm peer-[&:not(:placeholder-shown)]:-top-2 peer-[&:not(:placeholder-shown)]:text-xs">
            Email
          </label>
        </div>
        <div className="relative">
          <input 
            ...
            className="peer w-full rounded-md bg-white/5 border border-white/15 px-3 py-3 pr-10 text-white placeholder-transparent outline-none ring-1 ring-transparent focus:ring-2 focus:ring-green-500 focus:border-green-500 backdrop-blur-sm transition-shadow" 
            placeholder="Password" 
            autoComplete="current-password"
          />
          <label htmlFor="signin-password" className="pointer-events-none absolute left-3 top-3 text-sm text-neutral-800 transition-all bg-white px-1.5 rounded-md shadow-sm peer-focus:-top-2 peer-focus:text-xs peer-focus:text-emerald-600 peer-placeholder-shown:top-3 peer-placeholder-shown:text-sm peer-[&:not(:placeholder-shown)]:-top-2 peer-[&:not(:placeholder-shown)]:text-xs">
            Password
          </label>
          <button
            type="button"
      ...
      <div className={`mt-4 ${fullWidth ? 'w-full' : ''}`}>
        ...
        <div className="mt-3 text-sm text-neutral-300">
          ...
          <button type="button" onClick={onSwitchToSignUp} className="ml-2 text-green-400 hover:text-green-300 underline underline-offset-4">
            Create one
          </button>
        </div>
      </div>
