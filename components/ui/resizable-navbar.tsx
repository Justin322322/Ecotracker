...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    ...
    >
      <div
        className={cn(
          "w-full rounded-full px-4 py-2 flex flex-row items-center justify-between",
          "bg-black/80",
        )}
      >
        {children}
...
export const NavItems = ({ items, className, onItemClick }: NavItemsProps) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "absolute inset-0 hidden flex-1 flex-row items-center justify-center space-x-2 text-sm font-medium text-white/90 transition duration-200 hover:text-white lg:flex lg:space-x-2",
        className,
      )}
    >
      {items.map((item, idx) => (
...
export const NavbarLogo = () => {
  return (
    ...
    >
      ...
      <span className="font-medium text-white">EcoTracker</span>
    </a>
  );
};
...
export const NavbarButton = ({
  ...
  const variantStyles = {
    ...
    secondary: "bg-transparent text-white hover:text-green-400 border border-white/20 hover:border-green-400/50",
