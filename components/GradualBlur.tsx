...
const useIntersectionObserver = (ref: React.RefObject<HTMLDivElement>, shouldObserve: boolean = false) => {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(([entry]) => {
      if (entry) setIsVisible(Boolean(entry.isIntersecting));
    }, { threshold: 0.1 });
...
const GradualBlur: React.FC<GradualBlurProps> = props => {
  ...
  const blurDivs = useMemo(() => {
    ...
    for (let i = 1; i <= config.divCount; i++) {
      ...
      progress = (curveFunc as (p: number) => number)(progress);
  ...
  useEffect(() => {
    ...
    const ms = Number.parseFloat(String(duration)) * 1000;
    const t = setTimeout(() => onAnimationComplete(), Number.isFinite(ms) ? ms : 300);
